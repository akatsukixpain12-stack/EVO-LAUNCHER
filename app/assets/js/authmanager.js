...
/**
 * Add an Ely.by account. This authenticates credentials against Ely.by's
 * authserver and stores the returned session in the local config.
 *
...
exports.addElyByAccount = async function(username, password) {
    try {
        const response = await ipcRenderer.invoke('elyby-login', username, password)

        if(response?.success && response.uuid && response.accessToken) {
            ...
            if(ConfigManager.getClientToken() == null) {
                ConfigManager.setClientToken(require('crypto').randomUUID())
            }
            ConfigManager.save()
            return ret
        }
        ...
        return Promise.reject({
            ...
            desc: response?.error?.errorMessage || response?.error || 'Authentication failed.'
        })
    } catch (err) {
        log.error(err)
        return Promise.reject({
            title: 'Ely.by Login Failed',
            desc: 'An unknown error occurred while contacting Ely.by.'
        })
    }
}
...
/**
 * Remove an Ely.by account. Attempts to invalidate the remote token first,
 * then removes the local account entry regardless of the remote outcome.
 *
 * @param {string} uuid The UUID of the account to be removed.
 * @returns {Promise.<void>} Promise which resolves to void when the action is complete.
 */
exports.removeElyByAccount = async function(uuid){
    try {
        ...
        log.warn('Unable to invalidate Ely.by token before logout.', err)
    }
...
/**
 * Validate the selected account with Ely.by's authserver. Ely.by exposes a
 * refresh endpoint which returns a new access token when the current one is
 * still refreshable.
 *
 * @returns {Promise.<boolean>} Promise which resolves to true if the access token is valid.
 */
async function validateSelectedElyByAccount(){
    const current = ConfigManager.getSelectedAccount()
    const response = await ipcRenderer.invoke('elyby-refresh', current.accessToken, ConfigManager.getClientToken())
    ...
    if(response?.success && response.uuid && response.accessToken) {
        ConfigManager.addElyByAuthAccount(
            ...
            response.accessToken,
            current.username,
            response.username || current.displayName
        )
        ConfigManager.save()
        return true
    }

    log.info('Ely.by access token could not be refreshed.')
    return false
}
